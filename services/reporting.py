...
def build_pdf_report(data):
    ...
    role_names = data.get('role_names') or {'role1': 'Disputant 1', 'role2': 'Disputant 2'}
    task_background = data.get('task_background') or 'NO BACKGROUND INFO'
    story.append(Paragraph(f'Source file: <b>{filename}</b>', body_s))
    story.append(Paragraph(f"Roles: <b>{role_names.get('role1', 'Disputant 1')}</b> / <b>{role_names.get('role2', 'Disputant 2')}</b>", body_s))
    story.append(Paragraph(f'Task background: {task_background}', body_s))
    ...
    story.append(Paragraph(f'<b>Coach Summary:</b> {agreement}. Risk posture is <b>{risk["label"]}</b> with <b>{risk["negative_signals"]}</b> negative signals and <b>{risk["threats"]}</b> threats.', body_s))
    story.append(Spacer(1,0.08*inch))
    ...
    for t in turns:
        speaker = t.get('display_speaker') or (t.get('meta', {}) or {}).get('display_speaker') or t.get('speaker', 'Unknown')
        story.append(Paragraph(f"<b>[{speaker}]</b>  {t['text'][:500]}", body_s))
    story.append(Spacer(1,0.08*inch))
